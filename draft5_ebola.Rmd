...
ggplot(ebola_kikwit_1995, aes(x = as.Date(date), y = onset)) +
  theme_bw() +
  geom_col(fill = "steelblue", color = "black", width = 1.1) +
...
bar_plot_ebola <- ggplot(week_dat_2, aes(x = Week, y = onset)) +
  ...
  labs(x = "Weeks",
       y = "Count") +
  scale_x_continuous(breaks = seq(0, max(week_dat_2$Week), by = 5))+
  theme(
    axis.title.x = element_text(size = 24, family="Calibri"),
    axis.title.y = element_text(size = 24, family="Calibri"),
    axis.text = element_text(size = 24, family="Calibri")
...
outtt1<-DEoptim(neg_log_likelihood_constant, lower = c(0,0,0), upper = c(30,30,30), events = new_times_ebola[1:2],
                kernel = ray_kernel, 
                mu_fn = mu_fn, 
                delay = 10,
                mu_diff_fn = mu_diff_fn,
...
outtt2<-DEoptim(neg_log_likelihood_constant, lower = c(0,0,0), upper = c(30,30,30), 
                events = new_times_ebola[3:30], ##from day 61 until day 99
                kernel = ray_kernel, 
                mu_fn = mu_fn, 
                delay= 10,
                mu_diff_fn = mu_diff_fn,
...
## alpha1 = 0, delta1 = 16.32, A1 = 0.03358992
...
plot_days_ebola <- ggplot() +
  ...
  theme(axis.title.x = element_text(size = 20, family="Calibri"),
    axis.title.y = element_text(size = 20, family="Calibri"),
    axis.text = element_text(size = 20, family="Calibri"))+
  scale_x_continuous(limits = c(0, max_days * 1.05))
...
```{r}
true_pars_ebola <- list(Dayrange1 = list('Alpha' = alpha1,
                                         'Delta' = delta1,
                                         'A' = A1),
                        Dayrange2 = list('Alpha' = alpha2,
                                         'Delta' = delta2,
                                         'A' = A2),
                        Dayrange3 = list('Alpha' = alpha3,
                                         'Delta' = delta3,
                                         'A' = A3),
                        Dayrange4 = list('Alpha' = alpha4,
                                         'Delta' = delta4,
                                         'A' = A4))
new_pars_ebola <- list()
for(i in 1:50){
  new_pars_ebola$Dayrange1[[i]]<- result1[[i]]$optim$bestmem
  new_pars_ebola$Dayrange2[[i]]<- result2[[i]]$optim$bestmem
  new_pars_ebola$Dayrange3[[i]]<- result3[[i]]$optim$bestmem
  new_pars_ebola$Dayrange4[[i]]<- result4[[i]]$optim$bestmem
  
}

estimated_pars_ebola_dayrange1 <- do.call("rbind", new_pars_ebola$Dayrange1)  # Bind all estimated parameters into a matrix
estimated_pars_ebola_dayrange2 <- do.call("rbind", new_pars_ebola$Dayrange2)
estimated_pars_ebola_dayrange3 <- do.call("rbind", new_pars_ebola$Dayrange3)
estimated_pars_ebola_dayrange4 <- do.call("rbind", new_pars_ebola$Dayrange4)

estimated_pars_ebola_dayrange1 <- as.data.frame(estimated_pars_ebola_dayrange1)
estimated_pars_ebola_dayrange2 <- as.data.frame(estimated_pars_ebola_dayrange2)
estimated_pars_ebola_dayrange3 <- as.data.frame(estimated_pars_ebola_dayrange3)
estimated_pars_ebola_dayrange4 <- as.data.frame(estimated_pars_ebola_dayrange4)
library(ggplot2)
library(gridExtra)
...
# Create a list of true parameters for reference
true_pars <- c(alpha1, alpha2, alpha3, alpha4, delta1, delta2, delta3, delta4, A1, A2, A3, A4)

# Create list of data frames
estimated_pars_ebola_list <- list(estimated_pars_ebola_dayrange1, estimated_pars_ebola_dayrange2, 
                                  estimated_pars_ebola_dayrange3, estimated_pars_ebola_dayrange4)

# Create names for the plots
plot_names <- c("Dayrange1", "Dayrange2", "Dayrange3", "Dayrange4")

for(i in 1:4) {
  for(j in 1:3) {
    # Create a dataframe for plotting
    plot_df_ebola <- data.frame(Estimated_Value = unlist(estimated_pars_ebola_list[[i]][, j]))
    
    # Generate a density plot for each parameter
    p_ebola <- ggplot(plot_df_ebola, aes(x = Estimated_Value)) +
      geom_density(fill = "skyblue", alpha = 0.7) +
      geom_vline(aes(xintercept = true_pars[(i-1)*3 + j]), color = "red", linetype = "dashed") +
      labs(title = paste("Parameter:", names(true_pars_ebola[[plot_names[i]]])[j], "for", plot_names[i]),
           x = "Estimated Value",
           y = "Density") +
      theme_minimal()  # A clean theme
    
    # Store the plot in the list
    plot_list_ebola[[3*(i-1) + j]] <- p_ebola
  }
}

# Arrange the plots in a grid
grid_ebola <- do.call(grid.arrange, c(plot_list_ebola, ncol = 3))
...
# Create a list of the estimated parameters
estimated_pars_ebola_list <- list(Dayrange1 = estimated_pars_ebola_dayrange1, 
                                  Dayrange2 = estimated_pars_ebola_dayrange2, 
                                  Dayrange3 = estimated_pars_ebola_dayrange3, 
                                  Dayrange4 = estimated_pars_ebola_dayrange4)

# Add the day range and parameter names to each data frame, then bind them all together
estimated_pars_ebola_df <- lapply(names(estimated_pars_ebola_list), function(dayrange) {
  estimated_pars_ebola_list[[dayrange]] %>%
    mutate(Dayrange = dayrange) %>%
    rename(Alpha = par1, Delta = par2, A = par3) %>%
    pivot_longer(-Dayrange, names_to = "Parameter", values_to = "Estimated_Value")
}) %>%
  bind_rows()


# Similarly, restructure the true parameters to a data frame in tidy format
true_pars_ebola_df <- true_pars_ebola %>%
  enframe(name = "Dayrange", value = "Parameters") %>%
  unnest_wider(Parameters)
# Continue reshaping true_pars_ebola_df
true_pars_ebola_df <- true_pars_ebola_df %>%
  pivot_longer(cols = -Dayrange, names_to = "Parameter", values_to = "True_Value")


# Merge the estimated and true parameters into one data frame
pars_ebola_df <- left_join(estimated_pars_ebola_df, true_pars_ebola_df, by = c("Dayrange", "Parameter"))

# Generate the plots
p_ebola <- ggplot(pars_ebola_df, aes(x = Estimated_Value)) +
  geom_density(fill = "skyblue", alpha = 0.7) +
  geom_vline(aes(xintercept = True_Value), color = "red", linetype = "dashed") +
  facet_grid(Dayrange ~ Parameter, scales = "free") +
  labs(x = "Estimated Value", y = "Density") +
  theme_minimal()

# Save the plot to a PDF
ggsave("myplot_ebola.pdf", p_ebola, width = 15, height = 8.5)
...
# Generate the plots
b_ebola <- ggplot(pars_ebola_df, aes(x = "", y = Estimated_Value)) +
  geom_boxplot(fill = "skyblue", alpha = 0.7) +
  geom_hline(aes(yintercept = True_Value), color = "red", linetype = "dashed") +
  facet_grid(Dayrange ~ Parameter, scales = "free_y") +
  labs(x = "", y = "Estimated Value") +
  theme_minimal()

# Save the plot to a PDF
ggsave("boxplot_ebola.pdf", b_ebola, width = 15, height = 8.5)
```
...
# Create an empty list to store Rc values for all day ranges and all simulations
rc_list_ebola <- list()

for(i in 1:4) {
  # Calculate Rc for all simulations in the i-th day range
  rc_list_ebola[[i]] <- estimated_pars_ebola_list[[i]]$par1 / estimated_pars_ebola_list[[i]]$par2
}

# Combine all Rc values into a single data frame
rc_df_ebola <- bind_rows(lapply(1:4, function(i) {
  data.frame(Dayrange = plot_names[i], Estimated_Rc = rc_list_ebola[[i]])
}))

# Calculate true Rc values
true_Rc_ebola <- lapply(true_pars_ebola, function(x) {x$Alpha / x$Delta})

# Combine true Rc values into a single data frame
true_Rc_df_ebola <- bind_rows(lapply(1:4, function(i) {
  data.frame(Dayrange = plot_names[i], True_Rc = true_Rc_ebola[[i]])
}))

# Merge the estimated and true Rc values into one data frame
rc_ebola_df <- left_join(rc_df_ebola, true_Rc_df_ebola, by = "Dayrange")

# Generate the density plot
pnew_ebola <- ggplot(rc_ebola_df, aes(x = Estimated_Rc)) +
  geom_density(fill = "skyblue", alpha = 0.5) +
  geom_vline(aes(xintercept = True_Rc), color = "red", linetype = "dashed") +
  facet_wrap(~ Dayrange, scales = "free") +
  labs(title = "Comparison of Real and Estimated Rc Values",
       x = "Estimated Rc Value",
       y = "Density") +
  theme_minimal()
...
ggplot(rc_ebola_df[rc_ebola_df$Estimated_Rc<20,], aes(x = Dayrange, y = Estimated_Rc)) +
  geom_boxplot(fill = "lightgray", alpha = 0.7, varwidth = TRUE) +
  geom_hline(aes(yintercept = True_Rc), color = "red", linetype = "dashed") +
  labs(title = "Estimated Rc Values",
       x = "Day Range",
       y = "Rc") +
  theme_minimal()
...
new_times_ebola3 <- events_one_sim[31:277] 
...
new_times_ebola4 <- events_one_sim[278:292]
...
ggplot(data.frame(x = 1:length(new_times_ebola4) , y=cumulative_intensities_ebola4), aes(x = x, y = y)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  xlab("i") + 
  ylab(expression(Lambda(t[i]))) + 
...
inter_arrival_int_ebola <-c()
for(i in 1:length(cumulative_intensities_ebola1)-1){
  inter_arrival_int_ebola<-c(inter_arrival_int_ebola, cumulative_intensities_ebola1[i+1] - cumulative_intensities_ebola1[i])
}

uk<- 1-exp(-inter_arrival_int_ebola)
...
ggplot(df, aes(x = UniformRandomData, y = CalculatedIntensities)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red", size = 0.6) +
...
event_intensities_new <- mu_ts_new + conditional_intensity_list(times = new_events +1e-10, events = new_events, kernel = exp_kernel, parameters = list(alpha = 0.5,
                                 ...
                                 A = 2,
                                 delay = 0))
new_example_df_ray <- data.frame(t = new_events, intensity = event_intensities_new)


new_int_plot <- ggplot(new_int, aes(t, intensity)) + 
  theme_classic()+
            geom_line() + geom_point(data = new_example_df_ray, 
            aes(t, event_intensities_new)) +
  labs(x = "Time", y = expression(lambda(t)))+
  theme(axis.title = element_text(size = 20, family = "Calibri"),
        axis.text = element_text(size = 18, family = "Calibri"))




# Create a data frame with timepoints and event counts
df_time <- data.frame(Time = new_events, Event = 1:length(new_events))

# Create the plot
ggplot(df_time, aes(x = Time, y = Event)) +
  geom_step(color = "darkred") +
  labs(x = "Time", y = expression(N(t))) +
  theme_bw()+
  theme(axis.title = element_text(size = 20, family = "Calibri"),
        axis.text = element_text(size = 18, family = "Calibri"))
